// Full ISO-3166-1 alpha-2 country list used for the "Shop by origin" flag
// strip under the site header, and for resolving an ?origin= code back to a
// display name (e.g. on /shop). Codes match the same convention already
// used by Product.originCountry (set via the Origin Country dropdown on the
// seller Add Product form) and by lib/countryFlag.ts, so no conversion is
// needed anywhere this list is used against stored product data.
