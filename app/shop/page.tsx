                ...
                <Link href={`/shop/${p.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', cursor: 'pointer' }}>
                    ...
                    <div style={{ padding: '14px' }}>
                      ...
                      {p.reviews && p.reviews.length > 0 ? p.reviews.reduce((s: number, r: { rating: number }) => s + r.rating, 0) / p.reviews.length : 0 != null && (
                        <div style={{ color: 'var(--accent)', fontSize: '13px', marginBottom: '8px' }}>
                          {'\u2605'.repeat(Math.round(p.reviews && p.reviews.length > 0 ? p.reviews.reduce((s: number, r: { rating: number }) => s + r.rating, 0) / p.reviews.length : 0))} {p.reviews && p.reviews.length > 0 ? p.reviews.reduce((s: number, r: { rating: number }) => s + r.rating, 0) / p.reviews.length : 0}
