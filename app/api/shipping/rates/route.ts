...
export async function POST(request: NextRequest) {
  try {
    ...
    // Never show fake carrier names or UK-specific rates.
    ...
    // Group cart items by seller stripe account
    const sellerGroups = new Map<string, typeof cartItems>()
    for (const item of cartItems) {
      const key = item.sellerStripeAccountId ?? '__unknown__'
    ...
    for (const [stripeAccountId, items] of sellerGroups) {
      try {
        ...
        // Fetch product dimensions for this seller's items
        const productIds = items.map((i: { productId: string }) => i.productId).filter(Boolean)
        const products = productIds.length
          ? await prisma.product.findMany({
              where: { id: { in: productIds } },
              select: {
                id: true, weightGrams: true, lengthCm: true,
                widthCm: true, heightCm: true, price: true,
                originCountry: true,
              },
            })
          : []
        ...
        if (!combinedRates) {
          ...
          // Multi-seller: add cheapest seller cost to each option
