...
async function generateAndSend() {
  ...
  const money = (r: PromiseSettledResult<{ _sum: { total: number | null } }>) => {
    if (r.status !== 'fulfilled') return 'ERR'
    const v = r.value._sum.total ?? 0
  ...
  if (shippoMode === 'TEST')
    flagIssues.push('Shippo still on TEST key â request live key via portal Intercom')
  ...
  return resend.emails.send({
    ...
    subject: `Velor Daily Briefing â ${dateStr}`,
    html,
  })
}
