...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const updated = await prisma.sellerVerification.update({
    where: { id: params.id },
    data: { status: newStatus, verificationNotes: notes ?? null, reviewedAt: new Date() },
  });

  if (action === 'approve') {
    await prisma.seller.update({ where: { id: verification.sellerId }, data: { approved: true } });
  }
