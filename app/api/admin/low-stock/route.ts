...
function buildLowStockEmail(
  ...
  const subject = `Low Stock Alert ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ${count} product${count !== 1 ? 's' : ''} need restocking`;
  const html = `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
      ...
      <p style="color:#888;font-size:12px;margin-top:24px">Velor Marketplace ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ automated low-stock monitor</p>
    </div>
  `;
  return { subject, html };
}
  ...
  await prisma.agentLog.create({
    data: {
      ...
      details: JSON.stringify({
        ...
        products: flat.map((p) => ({ id: p.id, title: p.title, stock: p.stock })),
      }),
    },
  });
