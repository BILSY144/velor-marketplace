      ...
      {state !== 'disabled' && (
        <>
          <form ref={formRef} onSubmit={publish} style={{ ...card, marginBottom: '30px' }}>
            ...
            >
              {posting ? 'Saving…' : editingId ? 'Save changes' : 'Publish entry'}
            </button>
          </form>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {posts.map(p => (
              <article key={p.id} style={card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', alignItems: 'baseline', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    <button
                      type="button"
                      onClick={() => startEdit(p)}
                      style={{ background: 'none', border: 'none', padding: '6px', fontSize: '12px', color: 'var(--accent)', fontWeight: 700, textDecoration: 'underline', cursor: 'pointer' }}
                    >
                      Edit
                    </button>
                    <button
                ...
                <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', fontSize: '12.5px', alignItems: 'center' }}>
                  {p.videoUrl && <span style={{ color: 'var(--muted)' }}>▶ Video attached</span>}
                  {(p.makingProcess || p.notesTips || p.behindScenes) && (
                    <span style={{ color: 'var(--muted)' }}>
                      Sections: {[p.makingProcess && 'Making process', p.notesTips && 'Notes & tips', p.behindScenes && 'Behind the scenes'].filter(Boolean).join(' · ')}
                    </span>
                  )}
                  {(p.productIds?.length ?? 0) > 0 ? (
                    <span style={{ color: 'var(--accent)', fontWeight: 600 }}>
                      {p.productIds.length} shoppable {p.productIds.length === 1 ? 'listing' : 'listings'}: {p.productIds.map(productName).join(', ')}
                    </span>
                  ) : p.product ? (
