...
// Trolley payout setup -- TROLLEY-rail sellers only (added 2026-07-23
// evening, replacing Dots as the default rail for non-Stripe countries --
// see lib/payoutRail.ts and lib/trolley.ts). Mirrors the Payoneer/Dots
// setup pages' structure and rail guard: a STRIPE-rail seller who lands
// here is redirected to /dashboard/stripe-connect before anything renders;
// a legacy DOTS-rail seller is redirected to /dashboard/dots and a legacy
// PAYONEER-rail seller to /dashboard/payoneer instead. The rail comes live
// from /api/trolley/onboard, which resolves it from the seller's country --
// lib/payoutRail.ts, the single source of truth.
//
// HONESTY: while Trolley is not yet configured (no TROLLEY_ACCESS_KEY/
// TROLLEY_SECRET_KEY in Vercel -- Trolley's own KYC review of Velor's Bank
// Transfer Activation submission is still pending), the button registers
// interest and says so plainly -- it never claims Trolley payouts are live
// before they are.
...
export default function TrolleySetupPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch('/api/trolley/onboard')
      .then((r) => r.json())
      .then((d) => {
        if (cancelled) return;
        // Rail guard: a seller on any other rail must never see Trolley
  ...
  async function handleStart() {
    ...
    setPendingMsg('');
    try {
      const r = await fetch('/api/trolley/onboard', { method: 'POST' });
  ...
  return (
    <div style={{ ...pageStyle(), maxWidth: 760 }}>
      <StudioPageHead
        kicker="Money"
        title="Payout setup"
        sub={<>Your payout rail is <b>Trolley</b> — resolved from your country, automatically.</>}
      />

      <StudioNotice tone="blue">
        <b>Same rules, different rail.</b> Stripe does not support seller payouts in your country, so
        Velor pays you through Trolley instead. Delivery confirmation, hold windows and dispute freezes
        ...
        <>
          {status?.onboarded ? (
            <div style={cardStyle({ marginBottom: 14 })}>
              <div style={cardHeadStyle()}>
                <h3 style={{ fontSize: 13.5, fontWeight: 600, margin: 0 }}>Trolley account</h3>
              ...
              <p style={{ color: STUDIO.muted, fontSize: 13, lineHeight: 1.6, margin: 0, padding: '14px 18px 18px' }}>
                Your earnings are released to your Trolley account after each delivery is confirmed
            ...
            <div style={cardStyle({ marginBottom: 14, padding: '20px 22px' })}>
              ...
              <p style={{ color: STUDIO.muted, fontSize: 13, lineHeight: 1.6, margin: '0 0 16px' }}>
                ...
                Trolley&apos;s secure site — Velor never sees your bank details. You can sell and earn
          ...
          <div style={cardStyle({ padding: '20px 22px' })}>
            ...
            {[
              ...
              'Your share is released to your Trolley account — within 15 days for new sellers, 72 hours once trusted. An open return or dispute pauses release.',
            ].map((text, i) => (
