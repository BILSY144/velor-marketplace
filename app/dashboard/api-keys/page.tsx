...
<Link href="/dashboard/upgrade/enterprise" style={{ display: 'inline-block', background: 'var(--accent)', color: '#fff', padding: '12px 24px', borderRadius: '8px', textDecoration: 'none', fontWeight: 600 }}>
...
<h1 style={{ fontFamily: 'var(--font-display)', color: 'var(--text)', fontSize: '28px', marginBottom: '8px' }}>API Access</h1>
<p style={{ color: 'var(--muted)', marginBottom: '32px' }}>
Generate API keys to read your products programmatically. Keys are shown once at creation, so store them securely.
</p>

{revealedKey && (
<div style={{ background: 'var(--surface)', border: '1px solid var(--accent)', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
...
style={{ background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: '8px', padding: '12px 24px', fontWeight: 600, cursor: 'pointer', opacity: creating ? 0.6 : 1 }}
>
{creating ? 'Generating...' : 'Generate key'}
</button>
</form>

<div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', marginBottom: '32px' }}>
{keys.length === 0 ? (
