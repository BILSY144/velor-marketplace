...
/**
 * THE MAKERS' CIRCLE -- Velor's community hub (client half).
 *
...
/* ---------- real-data props (the three swapped sections) ---------- */
      ...
      <div className="mc-wrap">
        ...
        <SectionBox href="/community/featured" className="mc-span2">
          ...
          {featuredCards.length === 0 ? (
            <p className="mc-note">No maker is live or has posted fresh work yet &mdash; check back soon.</p>
          ) : (
                ...
                <article key={c.key} className="mc-fcard">
                  <div className="mc-fcard-media">
                    {c.img ? (
                      ...
                      <div className="mc-ph" aria-hidden="true" />
                    )}
                  </div>
                  <div className="mc-fcard-body">
                    ...
                    <div className="mc-fcard-country">
                      {c.cc && <span aria-hidden="true">{flagFor(c.cc)}</span>} {c.country}
                    </div>
        ...
        {/* ============ CREATOR JOURNALS + ASK THE MAKER -- real data ============ */}
        <div className="mc-row2">
            ...
            {journalPreview ? (
              <div className="mc-journal">
                <div className="mc-journal-text">
                  <h3 className="mc-journal-day">{journalPreview.title || 'Latest Entry'}</h3>
                  <p className="mc-journal-body">
                    {journalPreview.body.length > 160 ? `${journalPreview.body.slice(0, 160)}…` : journalPreview.body}
                  </p>
                  <div className="mc-journal-meta">
                    <span className="mc-journal-stat"><Ico d={PATHS.heart} size={13} /> {journalPreview.likes}</span>
                    <span className="mc-journal-stat"><Ico d={PATHS.comment} size={13} /> {journalPreview.comments}</span>
                  </div>
                </div>
                <div className="mc-journal-media">
                  {journalPreview.image ? (
                    <img src={journalPreview.image} alt="" loading="lazy" />
                  ) : (
                    <div className="mc-ph mc-ph-fill" aria-hidden="true" />
                  )}
                </div>
              </div>
            ) : (
              <p className="mc-note">No journal entries yet &mdash; the first one will appear here.</p>
            )}
          </SectionBox>
          ...
          <SectionBox href="/community/ask">
            ...
            {askRows.length === 0 ? (
              <p className="mc-note" style={{ marginBottom: 12 }}>No questions have been asked yet &mdash; be the first.</p>
            ) : (
              <div className="mc-ask-rows">
                {askRows.map((r) => (
                  <Link key={r.id} href={r.href} className="mc-ask-row" onClick={(e) => e.stopPropagation()}>
...
/* ask */
.mc-ask-search { display: flex; align-items: center; gap: 10px; background: var(--mc-card2); border-radius: 0; padding: 11px 18px; color: var(--mc-muted); font-size: 13.5px; margin-bottom: 12px; font-family: var(--font-display); }
