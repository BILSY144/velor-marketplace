...
POSSIBLE FOLLOW-UP, not yet actioned: other non-GB Stripe-rail sellers may be sitting on the same wrong-country GB accounts. A sweep (list sellers with stripeAccountId + country != GB, check each account's country via Stripe, reset the mismatched empty ones) would repair them proactively -- needs William's go-ahead.
