...
## OUTSTANDING -- ADD PAYONEER TO PAYMENT TRUST COPY EVERYWHERE (William, 2026-07-17, reminder set for 08:35 UTC)

William: add Payoneer to the "Secure Stripe checkout" trust copy in the header and footer "and everywhere its needed on website, mobile version and app". This intentionally revisits the 2026-07-13 decision that kept Payoneer OUT of buyer-checkout badges. The honesty constraint still applies: Payoneer Checkout (buyer payments) does not exist and Mass Payouts partner approval is still pending -- so the wording must be payout-framed (e.g. "Secure Stripe checkout · Payouts by Stripe & Payoneer"), never implying buyers pay via Payoneer. Sweep list: GlobalHeader topbar, GlobalFooter badges, /help, /sell, legal pages, checkout trust copy, and the app (CheckoutScreen, SellScreen, legal screens on mobile-app). Confirm exact wording with William before shipping.
