...
4. Set EASYSHIP_ORIGINS in Vercel (comma ISO codes) -- lanes switch on without deploys. Probe-verified candidate value: "GB,HK,US,CA,SG,FR,NL,DE,AU,NZ". IMPORTANT ORDERING: do NOT set non-Shippo origins (HK/US/SG/FR/NL/AU/NZ) before step 3 (card) is done -- an order on those lanes would attempt an Easyship label purchase with no payment method and no Shippo fallback. GB/DE/CA are safe anytime (Shippo fallback exists).
