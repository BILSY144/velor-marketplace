...
## Checkpoint 2026-07-08 -- Multilingual Velor (assistant plus outreach)

Buyer and seller assistant now speaks the visitor's language, and outreach emails are sent in the seller's own language. Every commit below was verified READY and PROMOTED on Vercel.

WHAT SHIPPED. app/api/assistant/chat/route.ts: LANG_RULE appended to BOTH BUYER_SYSTEM_PROMPT and BASE_SYSTEM_PROMPT, so the assistant always replies in the language the person wrote in; proper nouns (Velor, Stripe, Payoneer) stay as-is. Do not weaken this rule, because the outreach emails now promise it. components/VelorAssistant.tsx: 18-language I18N table, resolveLocale() reads navigator.language (no API call, zero cost), localized buyer greeting, input placeholder, and a "write to me in your own language" hint; dir=rtl for ar, ur, fa, he. lib/outreachI18n.ts (NEW): hand-written outreach copy in 19 languages (en es fr de it pt nl pl tr ru ar hi bn vi th id zh ja ko) plus langForCountry(), which maps ISO-2 codes and free-text locations to a language; unknown country falls back to English on purpose, because good English beats bad machine translation. lib/outreachEmail.ts (NEW): the SINGLE SOURCE OF TRUTH for the 3-touch founding-seller sequence, with localized subject, greeting, intro, benefit rows, CTA and sign-off, plus a highlighted "write to us in your own language" panel; RTL-aware; returns subject, html and lang. app/api/cron/outreach-auto/route.ts: resolves lang from prospect.country and logs the language distribution of each run to AgentLog details.languages. app/api/agents/outreach/route.ts: uses the localized builder and now passes prospect.country. app/api/agents/test-outreach/route.ts: uses the localized builder.

CLEANUP ITEM (not a bug). lib/email.ts still exports an old English-only buildOutreachEmail(). No caller imports it any more. Delete it when convenient, and do not add features to it.

NOT YET VERIFIED (per LAW #1). No localized outreach email has been observed delivered. Triggering one sends a real email to a real prospect, so it was not tested. The code compiles and is deployed. First live confirmation will come from the next outreach-auto cron run and its AgentLog languages field.
...
## SESSION UPDATE — 2026-07-08 (scheduled check-in, ~03:07 UTC)

Shipped since the 02:50 UTC update (HEAD was 195710a): ten commits landed on main, verified on the commits page. Headline is a 24-hour seller application SLA. Screening rules plus a shared provisioning module landed in 8fde669, an hourly seller onboarding agent enforcing the 24h decision promise in d2f9534, the admin review path now uses the same provisioning code as the agent (c749bb3), and an hourly review-applications cron was scheduled (32e9548). Site copy was corrected to match reality: the Sell page now says 190 countries and 24h approval instead of 22 and 48h (71ff52a), the About page now says 190 countries and 20 currencies (3fc5a1a), and the application-received email now promises a 24h decision instead of 3 to 5 business days (6fb82e8). Smaller polish: the drag-to-browse hint now shows on desktop too (4c7c02d) and the header and footer logos were resized to better match their bars (b78cfae, 58dbd07). All of these show passing checks on GitHub except the newest commit 58dbd07, which was showing 0 of 1 checks complete at time of writing and should be re-checked.

In progress and next: first live confirmation of a localized outreach email is still pending (check AgentLog languages on the next outreach-auto run), and the first live run of the hourly review-applications cron and onboarding agent should likewise be verified in AgentLog. The Payoneer partner application remains awaiting approval. The cj-resupplier build-failure re-verification from the 2026-07-07 emergency checkpoint remains outstanding. Nothing in this update touches or resolves any review item above.


## SESSION UPDATE — 2026-07-08 (scheduled check-in, ~03:47 UTC)

Shipped since the 03:07 UTC update (HEAD was 58dbd07; that update itself landed as 00c7887): five commits adding mandatory seller identity verification via Stripe Identity. The Apply page now states a decision within 24 hours (811fecb). SellerApplication gained identity verification columns (bd50bf3). A Stripe Identity adapter with a restricted-jurisdiction guard landed (0c31c18), the Stripe Identity webhook now accepts signature-verified verdicts only (95481c2), and no seller is approved without a verified identity document (d931305, HEAD at time of writing). All five show passing checks (1 of 1) on GitHub, and 58dbd07, which was flagged as 0 of 1 in the previous update, now shows 1 of 1 as well.

In progress and next: verify the first live end-to-end Stripe Identity verification in AgentLog once a real application flows through. The first live confirmation of a localized outreach email is still pending (check AgentLog languages on the next outreach-auto run), and the first live runs of the hourly review-applications cron and onboarding agent should likewise be verified. The Payoneer partner application remains awaiting approval. The cj-resupplier build-failure re-verification from the 2026-07-07 emergency checkpoint remains outstanding. Nothing in this update touches or resolves any review item above.


## Checkpoint 2026-07-08 (late) -- Identity verification, 24h SLA, mobile

Every item below was verified against a live deployment, a live DOM read, or the GitHub contents API. Per LAW #1, anything unconfirmed is marked as such.

SELLER IDENTITY VERIFICATION (new, the big one). Velor now accepts NO seller without a VERIFIED government-issued identity document. lib/identity.ts wraps Stripe Identity: stripe.identity.verificationSessions.create({ type: 'document' }) returns session.url, and webhooks identity.verification_session.verified / requires_input / processing / canceled report the verdict. Verified against docs.stripe.com/identity/use-cases and /verify-identity-documents on 2026-07-08. Stripe Identity is INDEPENDENT of Stripe Connect: it bills to Velor's own account, eligibility depends on VELOR's business location (GB is generally available), and it verifies documents from hundreds of countries. That means Payoneer-rail sellers -- who are by definition in countries Stripe Connect does not pay out to -- are still fully verifiable. Payout rail and verification rail are separate concerns.

RESTRICTED JURISDICTIONS, a legal constraint not a preference. Stripe's Identity terms forbid verifying anyone linked directly or indirectly with China, Russia, Cuba, Iran, North Korea, Syria, or Crimea/Donetsk/Luhansk, and anyone under 16. isRestrictedForIdentity() in lib/identity.ts blocks session creation for these, matching on both ISO-2 code and free-text country name because /apply stores names while other records store codes. Those applications get verificationStatus RESTRICTED, stay PENDING, and the seller receives an honest email saying it is a restriction placed on Velor, not a judgement about them. They wait for the Payoneer KYC rail: Payoneer runs an audited FATF/EU/US-standard KYC programme across 200 countries and exposes KYC webhooks via its white-label API, which is partner-gated and still awaiting approval (docs/PAYONEER_SETUP.md). Do not quietly approve a restricted-jurisdiction seller.

NO DOCUMENT EVER TOUCHES VELOR on the new path. Stripe hosts the capture, holds the images, and returns only a pass/fail. This is deliberate: a marketplace holding passport scans is a breach target and, with a selfie match, special-category biometric data under UK GDPR. WARNING, STILL OUTSTANDING: the OLD flow (app/api/seller/verify/route.ts, app/dashboard/verify/page.tsx, app/api/admin/verify/[id]/route.ts, SellerVerification.idDocumentUrl) still stores ID scans on Velor infrastructure and still exists. It also verifies nothing -- it collects documents, a human eyeballs them, and NO route gates listings, checkout or payouts on the result. Deleting it is the next job; do it carefully because app/seller/[sellerId]/page.tsx and app/api/briefing/route.ts reference SellerVerification.

BLOCKER, MUST BE DONE BY WILLIAM BEFORE ANY SELLER IS APPROVED. Stripe Identity is NOT enabled on the Velor Stripe account (acct_1TlcWCDB5eA3Wfmu). Checked 2026-07-08: dashboard.stripe.com/settings/identity and /identity/overview both redirect to the dashboard home, and the Add-destination event picker lists no identity.verification_session.* events. William must enable Stripe Identity (this means accepting Stripe's Identity Services Terms -- a legal agreement binding Velor Commerce Ltd, which Claude must not accept on his behalf), then create the webhook destination at https://velorcommerce.store/api/webhooks/stripe-identity subscribing to identity.verification_session.verified / requires_input / processing / canceled, then put the signing secret into Vercel as STRIPE_IDENTITY_WEBHOOK_SECRET. Claude must never enter that secret. Until then isIdentityConfigured() is false and the agent HOLDS every application rather than approving anyone unverified -- the correct failure mode, but it does mean zero approvals until the switch is flipped. First 50 Stripe Identity verifications are free, then USD 1.50 each.

24-HOUR SLA, now enforced in code rather than claimed in copy. lib/sellerApplicationReview.ts holds APPLICATION_SLA_HOURS = 24 and APPLICATION_ESCALATE_AFTER_HOURS = 12, plus screenApplication(), a pure synchronous screener against /legal/seller-rules (hard rejects for counterfeit, weapons, controlled substances, ivory/tortoiseshell, antiquities; objective completeness checks; unknown categories and regulated materials go to HOLD, never auto-approve). lib/provisionSeller.ts extracts approveApplication/rejectApplication so a human admin and the agent take exactly the same path -- there is no fast lane that skips account provisioning or emails. app/api/cron/review-applications/route.ts runs hourly (vercel.json, 11 crons now) and does, in order: screen; restricted-jurisdiction hold; create Identity session and email the seller a link; approve only when verificationStatus is VERIFIED; hold everything else. Only a VERIFIED application is on the 24-hour clock, because Velor cannot promise a decision speed while waiting on the seller's camera. Copy everywhere now reads "a decision within 24 hours of your verification completing".

HONEST-COPY FIXES, all verified live. The sell page claimed "22 countries reached" and "48h approval"; neither number was true of anything. It now reads 190 (the length of lib/worldCountries.ts, the same list the flag strip and the /apply dropdown use), Free, and 24h -- with APPLICATION_SLA_HOURS imported so the copy cannot drift from the code. The about page said "over 25 countries" and stat cards said 22 and 8; now 190 and 20 (SUPPORTED_CURRENCIES, counted on the live page). The application-received email and the /apply confirmation said "3-5 business days"; both now say 24 hours. Trust bar now reads "your money is held until delivery confirmation". Header logo 28px to 24px, footer 24px to 20px, and on phones the header logo is 16px via a .velor-headerbar img[alt='Velor'] rule inside the 768px media query.

FLAG STRIP REGRESSION, caused by Claude and fixed. Commit 92670fe ("Mount Velor AI assistant on all public pages") rewrote components/ConditionalLayout.tsx and silently dropped the CountryOriginStrip import. Nothing else imported it, so the shop-by-origin flag strip vanished from every public page and no build failed. Restored (ef14b9a) with a comment telling future sessions not to remove it. 190 flags confirmed rendering live. Lesson: when rewriting a whole file, diff the import list against the version you replaced.

MOBILE, and the reason it was broken. The old "mobile responsive layer" in app/globals.css targeted inline styles with attribute substring selectors, e.g. [style*='padding:0 40px']. Those never matched anything, ever: React serialises inline styles with a space after the colon and normalises units, so the DOM says "padding: 0px 40px". The entire mobile layer was dead code and the site rendered a squeezed desktop page on every phone. It is now keyed on real class names (velor-section, velor-hero, velor-h2, velor-grid-tiles, velor-grid-cats, velor-cta-row, velor-trustbar, velor-headerbar, velor-searchform, velor-right, velor-currency, velor-account-btn, velor-mobile-panel, velor-live-scroll), all confirmed attached to live DOM nodes. Search wraps to its own full-width row, currency and account move into the hamburger with 44px tap targets, tiles go 2-up, and the search input is 16px so iOS stops zooming on focus. NOT VERIFIED: Claude never saw the site at 390px -- resize_window does not move this Chrome's viewport and the site blocks iframing. Verified by code and by DOM class presence only. Ask William to eyeball it on a phone.

LIVE RAIL. components/DragScroller.tsx extracts the flag strip's pointer-drag + momentum (pointer capture, friction 0.94, manual tap hit-test against data-drag-href, native clicks swallowed so a fling never opens a stream). The homepage Velor Live Shopping rail uses it. Confirmed live: scrollLeft 0 to 150 on a synthetic drag, cursor grab. Momentum does not fire under synthetic PointerEvents, but the flag strip behaves identically under the same test, so that is the harness, not the code. A "Drag to browse." hint now shows on desktop too, because a grab cursor alone does not tell anyone the rail moves.

DEAD CODE REMOVED. lib/email.ts lost the old English-only buildOutreachEmail (431 to 305 lines); lib/outreachEmail.ts is the single source of truth. All ten surviving email builders confirmed intact by fetching the file at its commit SHA.

TOOLING LESSONS, each of which cost real time tonight. GitHub's CodeMirror editor: an "unsaved changes ... Restore/Discard" banner steals keyboard focus and shifts the layout, so arrow keys and typing silently go nowhere -- dismiss it first. Never put a javascript_tool call between the click that focuses the editor and the keys that navigate it; Runtime.evaluate steals focus. Never set "the last visible text input" by JS on an edit page: that is the FILENAME field and it renames the file. Clicking Commit and then navigating in the same batch can cancel the commit, so always re-read the contents API afterwards to confirm the file landed. mcp__workspace__web_fetch served a badly stale cached copy of prisma/schema.prisma (an ancient schema with no SellerApplication at all) while an in-browser fetch of the same URL returned the real file -- do not trust web_fetch for repo files. raw.githubusercontent /main is also stale; fetch by commit SHA.
