...
**Stripe account.updated webhook case shipped (commit `79421ef`) but Stripe is not actually sending this event to us -- confirmed live via Stripe's own API, not assumed.** The real gap: `Seller.stripeOnboarded` only ever refreshed when a seller's browser made it back to `/dashboard/stripe-connect/return` after Stripe's hosted onboarding (flagged, not fixed, in the 2026-07-13 payments audit). Added a `case 'account.updated'` to `app/api/stripe/webhook/route.ts` that mirrors `GET /api/stripe/connect/account`'s exact `charges_enabled && payouts_enabled` logic, plus a read-only diagnostic + guarded-fix admin route (`GET`/`POST /api/admin/stripe-webhook-status`, admin-gated same as every other `/api/admin/*` route).

Ran the GET diagnostic live and found **two separate real blockers**, checked directly against Stripe's API (`we_1ToBoGDB5eA3WfmuW5U3pbhP`, the real `velor-marketplace.vercel.app/api/stripe/webhook` endpoint):
1. `enabledEvents` is only `["payment_intent.succeeded", "checkout.session.completed"]` -- `account.updated` is not subscribed, so the new code never fires. Neither, apparently, are the events several EXISTING switch-cases in the same file already handle: `customer.subscription.created/updated/deleted`, `invoice.payment_failed`, `invoice.paid`, `payment_intent.payment_failed`. **Those cases may be dead code in production right now** -- not confirmed why (could be a stale event subscription list from an earlier version of this route, could be something else), but worth checking whether Pro subscription upgrades/downgrades and past-due handling are actually working via a different path, or not working at all. Not investigated further this session -- flagging for next session or for William to decide priority.
2. `application: null` on this endpoint -- meaning it was created WITHOUT `connect: true`, so it only ever receives events on the platform's own Stripe account, never on a connected (seller) account. Even with `account.updated` added to `enabledEvents`, a seller's own account finishing onboarding would still never reach this endpoint. Fixing this requires either a NEW Connect-scoped webhook endpoint (`connect: true`) or reconfiguring this one -- the admin route's POST deliberately refuses to do this automatically (returns 409) since creating/reconfiguring a live Stripe webhook endpoint is an account-settings change requiring William's explicit sign-off, not something to silently automate.

**Not fixed this session -- needs William's decision:** whether to (a) create a new Connect-scoped webhook endpoint for `account.updated` (and any other Connect events worth having), and (b) separately investigate/fix the missing subscription-lifecycle event subscriptions on the existing endpoint, which is arguably higher priority since it's been silently broken (if it is) for longer and affects real subscription revenue, not just a UX staleness gap.
